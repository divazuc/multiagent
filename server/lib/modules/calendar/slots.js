...
export function formatSlotsContext(slots, settings) {
  ...
  const lines = [...byDay.entries()].map(([date, times]) => {
    ...
    // The customer-facing form first; the ISO key in brackets is what calendar.book takes.
    return `- ${HEB_DAYS[d.getDay()]} ${hebDateDMY(date)} [${date}]: ${times.join(', ')}`;
  });
...
ציין/י את התאריך והיום בדיוק כפי שהם מופיעים ברשימה למעלה. אל תחשב/י בעצמך איזה יום בשבוע חל בתאריך.
כשכותבים תאריך ללקוח — תמיד בפורמט 31/08/2026 (יום/חודש/שנה), לעולם לא בצורה שבסוגריים המרובעים; הצורה שבסוגריים היא רק בשביל השורה של calendar.book.
כאשר הלקוח אישר מועד — ואם שמו כבר ידוע לך מהקשר העסק (למשל "שם הלקוח ידוע") אל תשאל/י אותו שוב, השתמש/י בו — הוסף/י בסוף התשובה שורה נפרדת בפורמט המדויק:
...
כשאת/ה שולח/ת ACTION — התשובה שלך מסתיימת בבקשת האישור בלבד. אל תכתוב/י "נתראה", "נקבע", "מחכה לך" או כל ניסוח שמשתמע ממנו שהפגישה כבר סגורה; המערכת מוסיפה הודעת אישור אוטומטית אחרי הביצוע, ולעיתים המועד נתפס בינתיים והבקשה נדחית — הכרזה מוקדמת תסתור את ההודעה שתגיע מיד אחריה.
אל תמציא/י פרטים על הפגישה שלא מופיעים כאן: לא מיקום, לא זום או לינק, לא משך אחר, ולא "אשלח פרטים בהמשך". אם הלקוח שואל איפה מתקיימת הפגישה ואין לך את המידע — אמור/י שנעדכן בהמשך, בלי לנחש.`;
}
