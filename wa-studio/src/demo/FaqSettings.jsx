...
// ════ Settings tab — hours + follow-up editable; bot policy admin-managed ════
    ...
    <div className="st-page">
      ...
      <section className="st-card">
        ...
        <div className="st-mode" role="radiogroup" aria-label="מצב הסוכן">
          ...
          <button className={mode === 'schedule' ? 'st-mode-btn on' : 'st-mode-btn'} onClick={() => setMode('schedule')}>
            <b>לפי שעות פעילות</b>
            <span>עונה רק בשעות שמוגדרות למטה</span>
          </button>
          <button className={mode === 'off' ? 'st-mode-btn st-mode-off on' : 'st-mode-btn st-mode-off'} onClick={() => setMode('off')}>
            <b>כבוי</b>
      ...
      <section className={mode === 'schedule' ? 'st-card' : 'st-card st-muted'}>
        ...
        <div className="st-days">
          {settings.days.map((d, i) => (
            <div key={i} className={d.active ? 'st-day' : 'st-day st-day-off'}>
        ...
        <textarea id="st-ahm" className="st-textarea" rows={2}
                  placeholder="למשל: תודה שפניתם! נחזור אליכם מיד עם פתיחת העסק 🙏"
      ...
      <section className="st-card st-locked">
        ...
        <div className="st-locked-note">
          מדיניות השיחה מנוהלת על ידינו כדי לשמור על עקביות ואיכות — שינוי בזרימת
